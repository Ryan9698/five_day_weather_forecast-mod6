...
function displayCurrentWeather(data, city) {
  ...
  weatherInfo.innerHTML = `
    <h2>${city}<img src="http://openweathermap.org/img/wn/${weather.icon}.png" alt="Weather Icon"></h2>
...
function displayFiveDayForecast(fiveDayForecast) {
  ...
  forecastContainer.innerHTML = '<h2 class="mt-4 text-center">5-Day Forecast</h2>';
...
function createCityButton(city) {
  ...
  if (Array.from(cityList.children).some(button => button.textContent === city)) {
...
function fetchWeatherData(city) {
  fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}`)
    ...
    .then(data => {
      const fiveDayForecast = data.list.filter(entry => new Date(entry.dt_txt).getHours() === 12);
